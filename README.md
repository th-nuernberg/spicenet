...
- [test_nir](/notebooks/spice_net_tutorial_test_nir.ipynb): This notebook tests exporting from SPICEnet in the original implementation to NIR and then reimporting it back to the original implementation. It also does some more very interesting things towards the end. We test HDF5 file export and import and also show that we can export and import multiple objects at the same time. We also show that our NIR architectures SPICENet class supports SPICEnets over multiple datastreams. This is a current theoretical exchange functionality that would allow SPICEnets over multiple data streams, for example IMU data as explored in some of the theoretical papers in this repository, to be exported into a single SPICEnet that holds multiple HCMs and their corresponding SOMs.
...
- [sinabs_transfer](/notebooks/sinabs_transfer.ipynb): This notebook tests the SINABS implementation of the SPICEnet based on the newly trained SOM neuron SNN. It included various analysis, transfer tests and evaluation comparisions. It also tries training directly in the new implementation. Careful: as some fitting on the SINABS version are tested running the whole notebook takes a few hours.
